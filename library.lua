...
local SOLAR_ICONS_URL = "https://raw.githubusercontent.com/Footagesus/Icons/refs/heads/main/solar/dist/Icons.lua"
local Solar = {}
do
    local compiler = loadstring or load
    if compiler then
        local ok, result = pcall(function()
            return compiler(game:HttpGet(SOLAR_ICONS_URL))()
        end)
...
local function SolarIcon(name, fallback)
    return Solar[name] or fallback or Solar["widget-2-bold"] or "rbxassetid://0"
end
...
local function AddRedBlackGradient(instance, rotation)
    if not instance or not instance:IsA("GuiObject") then return end
    instance.BackgroundColor3 = Color3.fromRGB(20, 0, 0)
    local gradient = instance:FindFirstChild("SolarRedBlackGradient")
    if not gradient then
        gradient = Instance.new("UIGradient")
        gradient.Name = "SolarRedBlackGradient"
        gradient.Parent = instance
    end
    gradient.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, Color3.fromRGB(120, 0, 0)),
        ColorSequenceKeypoint.new(0.48, Color3.fromRGB(38, 0, 0)),
        ColorSequenceKeypoint.new(1, Color3.fromRGB(0, 0, 0))
    })
    gradient.Rotation = rotation or 135
end

local function ApplySolarIcons(root)
    ...
    local iconMap = {
        ...
        home = "home-2-linear",
        menu = "widget-2-linear",
        add = "add-circle-linear",
        plus = "add-circle-linear",
        dropdown = "alt-arrow-down-linear",
    ...
    for _, object in ipairs(root:GetDescendants()) do
        if object:IsA("ImageLabel") or object:IsA("ImageButton") then
            local key = string.lower(object.Name .. " " .. object.Parent.Name)
            for token, iconName in pairs(iconMap) do
                if string.find(key, token, 1, true) then
                    SetImage(object, SolarIcon(iconName))
                    break
                end
            end
        end
...
local function AddFloatingLogoButton(screenGui, targetGui)
    if not screenGui or not targetGui then return end
    local old = screenGui:FindFirstChild("SolarFloatingLogo")
    if old then old:Destroy() end

    local button = Instance.new("ImageButton")
    button.Name = "SolarFloatingLogo"
    button.Size = UDim2.fromOffset(52, 52)
    button.Position = UDim2.new(1, -72, 1, -92)
    button.AnchorPoint = Vector2.new(0, 0)
    button.BackgroundColor3 = Color3.fromRGB(22, 0, 0)
    button.AutoButtonColor = false
    button.Image = SolarIcon("widget-2-bold")
...
function syde:Init(library)
	...
	if loaded == false then
		...
		AddRedBlackGradient(Library.main, 135)
		AddRedBlackGradient(window.top, 90)
		AddRedBlackGradient(window.pages, 135)
		ApplySolarIcons(Library)
		AddFloatingLogoButton(Library, Library)
		AddRedBlackGradient(Library.main, 135)
		AddRedBlackGradient(window.top, 90)
		AddRedBlackGradient(window.pages, 135)
		ApplySolarIcons(Library)
	...
	function tbdata:InitTab(tab)
		...
		local tdata = {
			...
			Key = tab.Key;
			Icon = tab.Icon or tab.IconName or "widget-2-linear";
		}
		...
		Tab.title.Text = tdata.Title
		Tab.Name = tdata.Title
		local tabIcon = Tab:FindFirstChild("icon") or Tab:FindFirstChild("Icon") or Tab:FindFirstChild("ImageLabel")
		if not tabIcon then
			tabIcon = Instance.new("ImageLabel")
			tabIcon.Name = "SolarTabIcon"
			tabIcon.BackgroundTransparency = 1
			tabIcon.Size = UDim2.fromOffset(18, 18)
			tabIcon.Position = UDim2.new(0, 8, 0.5, -9)
			tabIcon.Parent = Tab
		end
		SetImage(tabIcon, SolarIcon(tdata.Icon))
		if tabIcon:IsA("ImageLabel") or tabIcon:IsA("ImageButton") then
			tabIcon.ImageColor3 = syde.theme.Accent
		end
		if Tab.title:IsA("TextLabel") then
			Tab.title.Position = UDim2.new(0, 32, 0.5, -math.floor(Tab.title.Size.Y.Offset / 2))
		end
		...
		if tbdata.first then
			...
			tweenservice:Create(Tab, TweenInfo.new(2, Enum.EasingStyle.Quart), { Size = UDim2.new(0, Tab.title.TextBounds.X + 52,0, 35) }):Play()
			...
			tweenservice:Create(Tab, TweenInfo.new(2, Enum.EasingStyle.Quart), { Size = UDim2.new(0, Tab.title.TextBounds.X + 102,0, 35) }):Play()
...
function syde:SolarIcon(name, fallback)
    return SolarIcon(name, fallback)
end

function syde:ApplySolarIcons(root)
    ApplySolarIcons(root or Library)
end
